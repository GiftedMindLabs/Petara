...
const Profile: React.FC = () => {
  ...
  const activeTreatments = treatments.filter(
    (t) => t.petId === pet.id && t.status === "ongoing"
  );

  // Calculate total expenses
  const petExpenses = expenses.filter((e) => e.petId === pet.id);
  const totalExpenses = petExpenses.reduce((sum, e) => sum + e.amount, 0);
  const totalReimbursed = petExpenses.reduce(
    (sum, e) => sum + (e.reimbursed || 0),
    0
  );

  // Calculate age
  const age = Math.floor(
    (Date.now() - pet.birthDate) / (365.25 * 24 * 60 * 60 * 1000)
  );
          ...
          <View style={styles.infoList}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.value}>
                {nextVaccination
                  ? `${nextVaccination.name} on ${new Date(
                      nextVaccination.startDate
                    ).toLocaleDateString()}`
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.value}>
                {activeTreatments.length > 0
                  ? `${activeTreatments.length} ongoing treatment${
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoList}>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.value}>
                ${(totalExpenses - totalReimbursed).toFixed(2)}
              </Text>
            </View>
          </View>
        </View>
